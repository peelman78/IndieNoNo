...
//-----------------------------------------------------------------------------
//
// This is a modified version of teh plugin which change a few things:
//
//   -  Player movement is initiated on RELEASE of press/click, not on press/click.
//   -  The d-pad (and buttons if images are preesnted) will appear on a long press/click
//			at the pressed location.  For mobile devices only.
//
// Modified by Neil Collier, see: https://github.com/peelman78/IndieNoNo/tree/master/plugins
//
...
(function ($) {
	...
	Sprite_DirPad.prototype.hide = function() {
		...
		this.x = 0 - 64 - this.width * (1 + this.anchor.x);
	};
	...
	Sprite_DirPad.prototype.show = function() {
		...
		//this._moveSpeed = (dest - this.x) / this._moveDuration;
		this.x = $.Param.dpadPosition.x;
		this.y = $.Param.dpadPosition.y;
	};
	...
	Scene_Map.prototype.processMapTouch = function() {
		...
		if (!(this.isMobileDevice() && Scene_Base.dirpad)){
			Scene_Map_processMapTouch.apply(this, arguments);
		}

		if(this.isMobileDevice() && TouchInput.isLongPressed() && !Scene_Base.dirpad){
			//We are activating the d-pad
			MBS.MobileDirPad.Param.dpadPosition = new PIXI.Point(Number(TouchInput._x), Number(TouchInput._y));
			MBS.MobileDirPad.enable(true);
			return;
		}

		if (TouchInput.isReleased()){
				//We are releasing the d-pad
				if(Scene_Base.dirpad){
          MBS.MobileDirPad.enable(false);
        }
        else {

					if(!$.disallowMove){
						//This causes movement on release, instead of usually it does it on press
	          ...
	          $gameTemp.setDestination(x, y);
					...
					//Normal processing of map movement
					Scene_Map_processMapTouch.apply(this, arguments);
        }
		}
		else{
